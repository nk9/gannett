alter table "public"."metro_year_info" alter column "county_code" set data type text using "county_code"::text;
...
AS $function$
select
  d.id, m.id, my.county_code, m.name, d.name, m.state, y.year, ST_AsGeoJSON(d.geom) as geom
...
CREATE OR REPLACE FUNCTION public.lookup_district(_year integer, _state text, _county_code text, _district_name text)
 ...
 LANGUAGE sql
AS $function$
SELECT my.county, m.id, m.name, d.id, d.name, ST_PointOnSurface(d.geom::geometry)
...
WHERE _year = cy.year AND _state = m.state AND _county_code = my.county_code AND _district_name = d.name
LIMIT 10;
$function$
;
